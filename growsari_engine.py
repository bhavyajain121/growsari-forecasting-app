...
REQUIRED_DEMAND_COLS_ALIASES = {
    "warehouse_id": ["warehouse_id", "warehouse", "wh", "depot", "depot_id"],
    "sku_id":       ["sku_id", "sku", "skuid", "sku id"],
    "date":         ["date", "demand_date", "txn_date", "order_date"],
    "y":            ["y", "demand_units", "units", "qty", "quantity", "demand"],
}
...
REQUIRED_SKU_COLS_ALIASES = {
    ...
    "name":     ["name", "sku_name", "product_name"],
    "category": ["category", "cat", "product_category"],
}
COL_ALIASES = {
    "warehouse_id": ["warehouse_id", "warehouse", "wh", "warehouse id", "depot", "depot_id"],
...
def validate_and_normalize_demand(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if df["sku_id"].isna().any():
        raise ValidationError("Some rows have a non-numeric sku_id; please fix the source file.")
...
def validate_and_normalize_sku_master(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["sku_id"] = pd.to_numeric(df["sku_id"], errors="coerce")
    if df["sku_id"].isna().any():
        raise ValidationError("SKU master has rows with a non-numeric sku_id.")
    df["sku_id"] = df["sku_id"].astype(int)
...
def validate_combinations_upload(combos_df: pd.DataFrame, level: str, level_keys: list,
    ...
    if "sku_id" in df.columns:
        df["sku_id"] = pd.to_numeric(df["sku_id"], errors="coerce")
        n_bad = df["sku_id"].isna().sum()
        if n_bad:
            warnings.warn(f"{n_bad} combination row(s) had a non-numeric sku_id and were dropped.")
